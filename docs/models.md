...
| `tabpfn_bridge` | TabPFN-3 default regression checkpoint on quarterly log GDP growth, four GDP-growth lags and origin-known macro features: quarterly summaries or separate monthly slots/masks. Four fixed ensemble estimators, recursive horizons, training-row preprocessing. Research only. |
...
| `chronos2_covariates` | Chronos-2 median quarterly log growth conditioned on origin-known macro features: quarterly summaries/counts/masks or separate monthly slots/masks. Unavailable values stay NaN. Shares the existing Chronos checkpoint; no fine-tuning. |
...
For `chronos2_covariates`, the API's `future_covariates` argument refers to positions after the last released GDP quarter. Those positions may already have released monthly indicators at the actual origin. `--foundation-features quarterly` passes transformed partial-quarter means, counts and missing indicators. `monthly_slots` instead preserves each calendar month's transformed value and missing indicator in separate channels, without averaging. The same option applies to `tabpfn_bridge`. Unknown values stay NaN; no future releases, synthetic monthly GDP or native mixed-frequency measurement equation are implied. Transformations precede alignment and preserve native-calendar gaps. The adapter predicts log **growth**, whereas `chronos2` predicts log **levels**; differences between them cannot be attributed exclusively to covariates.
