...
The additional foundation adapters obey the same origin contract. TabPFN's bridge uses origin-vintage regression rows; TabPFN-TS and TimesFM 3 use GDP history only. TabPFN bridge and Chronos-2 covariates support `--foundation-features monthly_slots`: transform native series, then retain each month's calendar position and missing indicator separately. The default `quarterly` mode retains partial-quarter means/counts/masks. Chronos's API `future_covariates` means positions after the last GDP release; only values already known at the forecast origin may enter. Unknown values remain missing. All checkpoints must predate the origin and match their immutable publisher hashes. Restricted checkpoints require research mode. [Foundation instructions](foundation_models.md) and [data coverage](nowcast_data.md) explain the limitations.
