...
The additional foundation adapters obey the same origin contract. TabPFN's bridge uses origin-vintage regression rows; TabPFN-TS and TimesFM 3 use GDP history only. Chronos-2's covariate variant can receive API `future_covariates` only as origin-known partial-quarter summaries, counts and missing indicators after the last released GDP quarter; the API name does not authorize use of future macro releases. Unknown values remain missing. All checkpoints must predate the origin and match their immutable publisher hashes. Restricted checkpoints additionally require explicit research-use mode. [Foundation run instructions](foundation_models.md) explain checkpoint dates, licensing, and the limits of comparisons with earlier backtests.
