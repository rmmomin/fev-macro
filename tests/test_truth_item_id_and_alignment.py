...
def test_truth_item_id_matches_alfred_qoq_first_release(tmp_path: Path) -> None:
    release_csv = tmp_path / "gdpc1_releases.csv"
    pd.DataFrame(
        {
            "observation_date": ["2024-01-01", "2024-04-01", "2024-07-01"],
            ...
            "third_release": [101.0, 111.0, 122.0],
            "latest_release": [102.0, 112.0, 123.0],
            "qoq_growth_alfred_first_pct": [1.1, 2.2, 3.3],
            "qoq_growth_alfred_second_pct": [1.0, 2.0, 3.0],
            "qoq_growth_alfred_third_pct": [0.9, 1.9, 2.9],
        }
    ).to_csv(release_csv, index=False)
    ...
    assert out["item_id"].nunique() == 1
    assert str(out["item_id"].iloc[0]) == "gdpc1_qoq_first_pct"
    assert meta["item_id"] == "gdpc1_qoq_first_pct"
    assert meta["target_units"] == "pct_qoq"
    assert np.allclose(
        out["target"].to_numpy(dtype=float),
        np.array([1.1, 2.2, 3.3], dtype=float),
    )
...
def test_validate_y_true_matches_release_table_for_alfred_qoq_stages(tmp_path: Path) -> None:
    release_csv = tmp_path / "gdpc1_releases.csv"
    pd.DataFrame(
        {
            "observation_date": ["2024-01-01", "2024-04-01", "2024-07-01"],
